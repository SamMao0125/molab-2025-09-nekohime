...
//    @Published var mapRegion = MapRegionModel()
    ...
    func signIn() {
        print("LobbyModel signIn ฅ(^・ω・^ฅ)", state)
        if GIDSignIn.sharedInstance.hasPreviousSignIn() {
            print("LobbyModel signIn restorePreviousSignIn (=^･ω･^=)", state)
    ...
    func authenticateUser(for user: GIDGoogleUser?, with error: Error?) {
        if let error = error {
            print("authenticateUser error (=；ェ；=)", error.localizedDescription)
            return
        }
        ...
        Auth.auth().signIn(with: credential) { [unowned self] (_, error) in
            if let error = error {
                print("authenticateUser error 2 (=；ェ；=)", error.localizedDescription)
            } else {
                setSignedIn()
            }
        }
...
//            }
    ...
    func setSignedIn() {
        print("setSignedIn state ฅ(^・ω・^ฅ)", state)
        if state != .signedIn {
            state = .signedIn
            print("setSignedIn set signedIn (=^･ω･^=)", state)
            updateCurrentUser()
...
//            app.galleryModel.refresh()
    ...
    func signOut() {
        ...
        do {
            ...
            state = .signedOut
            currentUser = nil
            print("signOut success (=｀ω´=)")
        } catch {
            print("signOut error (=；ェ；=)", error.localizedDescription)
        }
    }
    ...
    func updateCurrentUser() {
        ...
        print("updateCurrentUser id ฅ(^・ω・^ฅ)", id)
        // print("value", values);
...
//        }
        ...
        lobbyRef.getData() { error, snapshot in
            guard error == nil else {
                print("postUser getData error (=；ェ；=)",error!.localizedDescription)
                return;
            }
            // print("updateCurrentUser snapshot", snapshot ?? "-nil-");
            if let users = snapshot?.value as? [String: Any] {
                if let user = users[id] as? [String: Any] {
                    print("updateCurrentUser user uploadCount (=^･ω･^=)", user["uploadCount"] ?? "-nil-");
                    values["uploadCount"] = user["uploadCount"] as? Int ?? 0
                    values["activeCount"] = ServerValue.increment(1)
                    // Preserve existing catMood if it exists
                    if let existingMood = user["catMood"] as? String {
                        values["catMood"] = existingMood
                    } else {
                        values["catMood"] = CatMood.available.rawValue
                    }
                }
            }
            // Set default mood for new users
            if values["catMood"] == nil {
                values["catMood"] = CatMood.available.rawValue
            }
            ...
            lobbyRef.child(id).updateChildValues(values) { error, ref in
                if let error = error {
                    print("postUser updateChildValues error (=；ェ；=): \(error).")
                }
            }
    ...
    func updateUser(user: UserModel) {
        print("updateUser updateUser ฅ(^・ω・^ฅ)", user.name)
        guard let lobbyRef else { return }
        
        var values:[String : Any] = [:]
        values["caption"] = user.caption
        values["stats"] = user.stats
        values["catMood"] = user.catMood.rawValue
        
        lobbyRef.child(user.id).updateChildValues(values) { error, ref in
            if let error = error {
                print("updateUser updateChildValues error (=；ェ；=): \(error).")
            } else {
                print("updateUser success ฅ(^・ω・^ฅ) mood:", user.catMood.rawValue)
            }
        }
    }
    
    // Update only the cat mood for current user
    func updateCurrentUserMood(_ mood: CatMood) {
        guard let lobbyRef else { return }
        guard let currentUser else { return }
        
        let values: [String: Any] = ["catMood": mood.rawValue]
        
        lobbyRef.child(currentUser.id).updateChildValues(values) { error, ref in
            if let error = error {
                print("updateCurrentUserMood error (=；ェ；=): \(error).")
            } else {
                print("updateCurrentUserMood success", mood.kaomoji, mood.rawValue)
                // Update local current user
                self.currentUser?.catMood = mood
            }
        }
    }
    ...
    func observeStart() {
        guard let lobbyRef else { return }
        print("observeUsersStart usersHandle ฅ(^・ω・^ฅ)", lobbyHandle ?? "nil")
        if lobbyHandle != nil {
            return;
        }
        lobbyHandle = lobbyRef.observe(.value, with: { snapshot in
            ...
            guard let snapUsers = snapshot.value as? [String: [String: Any]] else {
                print("LobbyModel users EMPTY (=；ェ；=)")
                self.users = []
                return
            }
            ...
            if let user = self.currentUser {
                if let nuser = sortedItems.first(where: { $0.id == user.id } ) {
                    print("LobbyModel currentUser nuser (=^･ω･^=)", nuser.catMood.rawValue)
                    self.currentUser = nuser
                }
            }
            print("LobbyModel users.count ฅ(^・ω・^ฅ)", self.users.count, "cats")
//            self.locsForUsers(firstLoc: nil)
    ...
    func observeStop() {
        guard let lobbyRef else { return }
        print("observeUsersStop usersHandle (=｀ω´=)", lobbyHandle ?? "nil")
